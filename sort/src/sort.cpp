...
namespace sort{
	...
	int mergesort(float* A, int i, int j, int k){
		float* B;
		float* C;
		int i1 = 0, i2 = 0;
		// el siguiente for hace la tarea de dividir todo en 2 sublistas
		for(int s; s<k; s++){
			if (s >= i && s<=j){
				if (i2 != 1) i1 = 0;
				B[i1] = A[s];
		...
		// el siguiente for hace la tarea de ordenar las sublistas
